...
async function handleRequest(event) {
  if(event.request.headers.get("requestMode") === "generate"){
    ...
    const queuebrokerRequestClone = queuebrokerRequest.clone()
    var queuebrokerRequestCloneBody = await queuebrokerRequestClone.json()
    queuebrokerRequestCloneBody["GitHubTokenTest"] = "1223"

    queuebrokerRequestCloneBody["GitToken"] = await generateGitJWT(queuebrokerRequestCloneBody["sessionID"])
...
async function generateGitJWT(sessionID){
  ...
  const test = await TEAMS.getWithMetadata("james1")
  console.log(test)
